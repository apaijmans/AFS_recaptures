...
#~~ Preprocessing
preprocessing(msat %>% select(-Best_genotype))

# Test data:
#msat <- msat[c(10:20, 270:280),] 

#~~ Generate datasets to compare
msat1 <- generateData1(msat %>% filter(Best_genotype == 0) %>% select(-Best_genotype))
msat2 <- generateData2(msat %>% select(-Best_genotype))
