...
checkNecessary <- function(id1, id2, cfg, id_to_group) {
  ...
  if (!cfg$checkOnlyPuppies || cfg$compareWithPup) {
    ...
    # if both IDs already belong to a group
    if (exists(id1, envir = id_to_group) &&
        exists(id2, envir = id_to_group)) {
      
      # and if they are already in the same group, skip
      if (id_to_group[[id1]] == id_to_group[[id2]]) return(FALSE)
    }
...
compareEntry <- function(row1, row2, loci_9, range_data,
  ...
  # mismatch_counter_loci_9 <- length(loci_9) - threshold_loci_9
  # 
  # for (i in loci_9) {
  #   if (!is.na(v1[i]) && !is.na(v2[i]) && v1[i] == v2[i]) { # True matches: both non-NA and equal
  ...
  #     
  #     if (!is.na(v1[i]) && !is.na(v2[i]) && v1[i] != v2[i]) { # True mismatches: both non-NA and NOT equal
  #       shared_counter_loci_9 <- shared_counter_loci_9 + 1 
  #     }
  #     
  #     if (mismatch_counter_loci_9 < 0) return(0) #max number of mismatches reached score 0 given back to compareData 
  #   }
  # }
  
  # --- loci 9 (vectorised with early-exit simulation) ---
  r1 <- v1[loci_9]
  r2 <- v2[loci_9]
  ...
  # Logical vectors
  both_non_na <- !is.na(r1) & !is.na(r2)
  matches_logical <- both_non_na & (r1 == r2)
  mismatches_logical <- !matches_logical  # includes NA mismatches
  
  # Count mismatches cumulatively
  cumulative_mismatches <- cumsum(mismatches_logical)
  
  # Early exit: if cumulative mismatches exceed allowed, return 0
  if (any(cumulative_mismatches > allowed_mismatches_9)) return(0)
  
  # Shared loci = matches + non-NA mismatches
  shared9 <- sum(matches_logical) + sum(both_non_na & r1 != r2)
  matches9 <- sum(matches_logical)
...
compareData <- function(msat1, msat2, cfg, pp,
  ...
  id_to_group <- new.env(hash = TRUE, parent = emptyenv())
  ...
  for (i in seq_len(nrow(msat1))) {
    ...
    for (j in seq_len(nrow(msat2))) {
      ...
      if (!checkNecessary(id1, id2, cfg, id_to_group)) next
      ...
      if (scores[1] > 0) {
        ...
        if (!exists(id1, envir = id_to_group) &&
          ...
          # Preserve order
          group_order <- c(group_order, id1)
          
        } else if (exists(id1, envir = id_to_group)) {
  ...
  lapply(group_order, function(x) results_env[[x]])
...
msat2 <- generateData(msat %>% 
                        ...
                        select(-Best_genotype), cfg, mode = "data2") # compare to best genotypes
